...
export class BazelBuildCodeLensProvider implements vscode.CodeLensProvider {
  ...
  private computeCodeLenses(
    ...
    // Sort targets by name length
    // This is a heuristic to achieve that the main target generated by marcros will also be the first shown by the codelens
    const sortedTargets = [...queryResult.target].sort(
      (a, b) => a.rule.name.length - b.rule.name.length,
    );
